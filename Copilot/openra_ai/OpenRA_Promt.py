...
class TargetsQueryParam:
    ...
    '''
    location: Optional[Location] = None  # 仅用于配合distance约束使用，会判断这个点和目标的距离，无distance的restrain时不需要填入
    direction: Optional[str] = None  # {ALL_DIRECTIONS} 中的一个或 None，仅用于配合maxnum使用，会选择所有满足条件的单位，最靠近direction的maxnum个单位，无maxnum的restrain时不需要填入。
