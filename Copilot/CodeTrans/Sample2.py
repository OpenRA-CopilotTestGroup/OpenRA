...
def lure_and_attack_sample():
    ...
    while True:
        # 检测有没有碰到人
        enemies_near_motorcycle = api.query_actor(OpenRA.TargetsQueryParam(faction="敌方", location=base_position, restrain=[{"distance": 50}]))

        if enemies_near_motorcycle:
            # 有人就往初始位置跑
            retreat_path = api.find_path([jeep], initial_position, '最短路径')
            if retreat_path["path"]:
                intermediate_position = retreat_path["path"][len(retreat_path["path"]) // 2]
